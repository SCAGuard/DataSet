...
int main()
{
  ...
  for (size_t byte = 0; byte < 256; byte += 16)
  {
   ...
   int te0 = 0x228d40;
    ...
    for (probe = base + te3; probe < base + te0 + 63*16; probe += 64) // hardcoded addresses (could be done dynamically)  //    for (probe = base + te3; probe < base + te0 + 64*16; probe += 64) // hardcoded addresses (could be done dynamically)
    {
      size_t count = 0;
