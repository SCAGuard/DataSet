...
char spy()
{
    ...
    for (i = 0; i < L1_NUM_SETS; i++) {
        eviction_set_addr = get_eviction_set_address(spy_array, i, 1); //- causes seg fault when not spy_array or trojan_array  //        eviction_set_addr = get_eviction_set_address(spy_array, i, 0); //- causes seg fault when not spy_array or trojan_array
        RDTSC(start);
        while(eviction_set_addr != NULL){
