...
void trojan(char byte)
{
	...
	* discover correct address for data point, place incorrect eviction_set address
     */
    eviction_set_addr = get_eviction_set_address(trojan_array, set, 1); //added this - causes seg fault when removed  //    eviction_set_addr = get_eviction_set_address(trojan_array, set, 0); //added this - causes seg fault when removed
    //while p != null, p = *p;
